...
test('模拟夹爪仅在确认解锁和明确提交后执行动作', async ({ page }) => {
  ...
  await page.route('**://*/api/**', async (route) => {
    ...
    if (path === '/api/grippers/sim-gripper/commands') {
      ...
      currentStatus = { ...currentStatus, position: 650, armed_until: Date.now() / 1_000 + 60 }
      await fulfillJson(route, { idempotency_key: request.headers()['idempotency-key'], replayed: false, status: currentStatus })
      return
    }
  ...
  await page.getByRole('button', { name: '夹爪控制' }).click()
  await expect(page.getByRole('heading', { name: '夹爪控制' })).toBeVisible()
  ...
  await page.getByLabel('目标位置数值').fill('650')
  await page.getByLabel('目标力数值').fill('25')
  await page.getByLabel('速度数值').fill('10')
  expect(commands).toHaveLength(0)

  await page.getByRole('button', { name: '执行目标位置' }).click()
...
test('真机模式禁用未验证的速度和软件停止', async ({ page }) => {
  ...
  await expect(page.getByText('真机 TCP')).toBeVisible()
  await expect(page.getByText('已设定目标位置')).toBeVisible()
  await expect(page.getByText('实时位置反馈未验证')).toBeVisible()
  await expect(page.getByRole('slider', { name: '速度' })).toBeDisabled()
  await expect(page.getByRole('button', { name: '停止仿真动作' })).toHaveCount(0)
  await expect(page.getByText(/真机无软件停止接口/)).toBeVisible()
})
...
test('旧版预览服务缺少夹爪接口时提示受控重启', async ({ page }) => {
  ...
  await expect(page.getByText('夹爪控制接口不可用，请重启服务。')).toBeVisible()
  await expect(page.getByText('未配置夹爪')).toHaveCount(0)
})
...
test('当前服务成功返回空夹爪列表时显示未配置状态', async ({ page }) => {
  ...
  await expect(page.getByText('未配置夹爪')).toBeVisible()
