...
网页预览 Plugin 使用独立的 `PluginHost`，不加入 `Runtime` 指令调度图。它只接收 JPEG 发布后的 `FrameCaptured` 事件，不能访问执行目标、适配器客户端或人工控制门面。当前受信任注册表只包含 `visual-pose-analysis`，负责维护姿态和成像分析快照；新增网页 Plugin 必须先在服务端注册固定工厂。采集循环、JPEG 缓存和 MJPEG 流不因 Plugin 刷新或重载而重建。重载只允许 `development` 模式下的本机回环服务，且必须由 `web.plugin_reload_enabled` 显式开启；生产模式始终要求完整进程重启。
